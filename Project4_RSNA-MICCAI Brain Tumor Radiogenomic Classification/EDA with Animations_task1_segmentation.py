    ...
    def __getitem__(self, index):
        ...
        for t in ("FLAIR", "T1", "T1CE"): # "T2w"
            ...
            for idx in range(len(t_path)):
                file_name = t_path[idx].split('\\')[0]
                index_num = t_path[idx].split('\\')[-1].split('.')[0][len(t):]
                num = re.sub(r'[^0-9]', '', index_num)
                t_path_num.append(num.zfill(10))
            t_path_num.sort()
            new_t_path=[]
            for idx in range(len(t_path)):
                new_t_path.append(os.path.join(file_name,t+str(int(t_path_num[idx]))+'.png'))
            ...
            min_index=154
            max_index=0
            for idx, image_path in enumerate(new_t_path):
            ...
            for i in range(start_num, end_num):
                channel.append(cv2.resize(load_image_normalization(new_t_path[i]), (256, 256)) / 255)
...
class Trainer:
    ...
    def fit(self, epochs, train_loader, valid_loader, save_path, patience):
        for n_epoch in range(1, epochs + 1):
            ...
            state_saver = saver.create(conf=None, model=model,optimizer=optimizer,checkpoint_prefix='check_point',recovery_prefix='recovery',
                                       checkpoint_dir='checkpoint',recovery_dir='recovery',max_history=10)
            state_saver.save_checkpoint(n_epoch,valid_score)
            if True:
